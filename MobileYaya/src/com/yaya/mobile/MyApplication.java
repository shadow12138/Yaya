...
public class MyApplication extends Application {
	...
	public void onCreate() {
		...
		initImageLoader();
		com.yaya.mobile.Config.setLocalDir(getPackageName());
		super.onCreate();
	...
	private void initImageLoader() {
		...
		ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(this).threadPriority(Thread.NORM_PRIORITY - 2).memoryCache(new WeakMemoryCache()).denyCacheImageMultipleSizesInMemory()
				.diskCacheFileNameGenerator(new Md5FileNameGenerator()).diskCacheSize(100 * 1024 * 1024)

				// 100 Mb
				.diskCacheFileCount(200).diskCache(new UnlimitedDiskCache(meCacheDir)).tasksProcessingOrder(QueueProcessingType.LIFO).writeDebugLogs() // Remove
																																						// for
																																						// release
																																						// app
				.build();
	...
	public static DisplayImageOptions getOptions() {
		if (mOptions == null) {
			mOptions = new DisplayImageOptions.Builder().showImageOnLoading(R.drawable.icon_stub).showImageForEmptyUri(R.drawable.icon_empty).showImageOnFail(R.drawable.icon_error)
